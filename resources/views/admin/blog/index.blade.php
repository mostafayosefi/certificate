@component('admin.layouts.content',[
    'title'=>'مشاهده بلاگها',
    'tabTitle'=>'مشاهده بلاگها ',
    'breadcrumb'=>[
            ['title'=>'مشاهده بلاگها','class' => 'active']
    ]])
    ...
    <div class="card-body">
         <table class="table table-separate table-head-custom table-foot-custom table-checkable" id="kt_datatable">
            ...
            <tbody>
                ...
                @foreach($blogs as $key => $admin)
                <tr>
                    <td>{{ $key + 1 }}</td>
<td>{{$admin->title}}</td>
<td>{{ date_frmat($admin->created_at) }}</td>
                        <td nowrap="">
                            <a href="{{ route('admin.blog.edit', $admin) }}" class="btn btn-icon btn-outline-primary btn-circle btn-sm mr-2">
                                <i class="flaticon2-pen"></i></a>
                                @php $admin['title'] = $admin->title;  @endphp
                                @include('admin.layouts.table.modal', [$admin ,'route' => route('admin.blog.destroy', $admin)  , 'myname' => $admin->title ])
                        </td>
                    </tr>
                @endforeach
