@component('admin.layouts.content', [
    'title' => 'مشاهده لوگوهای فوتر',
    'tabTitle' => 'مشاهده لوگوهای فوتر ',
    'breadcrumb' => [['title' => 'مشاهده لوگوهای فوتر', 'class' => 'active']],
    ])
     ...
     <div class="card card-custom">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="card-label"> لیست لوگوهای فوتر  </h3>
            </div>
            <div class="card-toolbar">

                 <a href="{{ route('admin.award.create') }}" class="btn btn-primary font-weight-bolder">
                    <i class="la la-plus"></i>ثبت لوگو فوتر </a>
             </div>
        </div>
        <div class="card-body">
             <table class="table table-separate table-head-custom table-foot-custom table-checkable" id="kt_datatable">
                ...
                <tbody>
                    ...
                    @foreach ($awards as $key => $admin)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $admin->title }}</td>
                            <td>{{ date_frmat($admin->created_at) }} </td>
                            <td nowrap="">
                                <a href="{{ route('admin.award.edit', $admin) }}" class="btn btn-icon btn-outline-primary btn-circle btn-sm mr-2">
                                    <i class="flaticon2-pen"></i></a>
                                    @php $admin['title'] = $admin->name;  @endphp
                                    @include('admin.layouts.table.modal', [$admin ,'route' => route('admin.award.destroy', $admin)  , 'myname' => ' لوگو فوتر '.$admin->title ])
